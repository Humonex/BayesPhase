...
class MethBridger(Bridger):
    ...
    class Gap(object):
        ...
        def bridge(self):
            ...
            # 两套方法独立评估：
            # 甲基化-only结果保留不变；joint方法从所有未定相read重新开始，不再只rescue甲基化失败read
        ...
        def phase_reads_bayes_beta_snp(self, snps, unphased):
            # SNP独立判别：仅使用SNP位点进行Beta-Bayes后验累加
            phasing = []
            if not snps:
                return phasing
            ...
            for read in unphased:
                ...
                for rloc, ref, alt, read_base, support_type in MethBridger.collect_snps_in_read(read, snp_sites):
                    ...
                    for ps, hp, alpha, beta in haps:
                        logp = self.beta_log_likelihood(sval, alpha, beta)
                        posterior[(ps, hp)] += logp
            ...
            for read in unphased:
                ...
                if len(snp_sites) > 0:
                    for rloc, ref, alt, read_base, support_type in MethBridger.collect_snps_in_read(read, snp_sites):
                        ...
                        sval = 1.0 if support_type == "ALT" else 0.0
                        for ps, hp, alpha, beta in haps:
                            snp_posterior[(ps, hp)] += self.beta_log_likelihood(sval, alpha, beta)
        ...
        def get_snp_posterior_detail_in_read(self, read, snps):
            ...
            for rloc, ref, alt, read_base, support_type in MethBridger.collect_snps_in_read(read, snp_sites):
                ...
                if len(haps) > 0:
                    informative_locs.add(rloc)
                for ps, hp, alpha, beta in haps:
                    posterior[(ps, hp)] += self.beta_log_likelihood(sval, alpha, beta)
            ...
            for read in self.bf.fetch(self.blk.chrom, self.blk.start(), self.blk.end(), multiple_iterators=True):
                ...
                if snps:
                    ...
                    for rloc, _, _, _, st in MethBridger.collect_snps_in_read(read, snp_sites):
                        ...
                        if rloc in site_betas:
                            sval = 1.0 if st == "ALT" else 0.0
                            obs.append((rloc, sval))

                if len(obs) < 5: continue
                obs.sort(key=lambda x: x[0])

                # 无 switch 全局 log-likelihood
                logp_no = sum(self.beta_log_likelihood(val, *site_betas[rloc][hp_current])
                              for rloc, val in obs if rloc in site_betas)
                ...
                for p in positions:
                    ...
                    for rloc, val in obs:
                        ...
                        logp_switch += self.beta_log_likelihood(val, *site_betas[rloc][hp_use])
