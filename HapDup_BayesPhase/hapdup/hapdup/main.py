...
def main():
    ...
    for e in [SAMTOOLS, FLYE, MINIMAP]:
    ...
    pepper_vcf = os.path.abspath(os.path.join(pepper_dir, "intermediate_files/PEPPER_VARIANT_FULL.vcf.gz"))

    margin_dir = os.path.join(args.out_dir, "margin")
    bridge_dir = os.path.join(args.out_dir, "bridge")
    whatshap_dir = os.path.join(args.out_dir, "whatshap")
    ...
    bridge_vcf = os.path.join(bridge_dir, "bridge") + ".vcf"
    bridge_vcf_gz = os.path.abspath(os.path.join(bridge_dir, "bridge.vcf.gz"))
    bridge_haplotagged_bam = os.path.join(bridge_dir, "bridge") + ".haplotagged.bam"
    whatshap_bridge_haplotagged_bam = os.path.join(bridge_dir, "whatshap_bridge") + ".haplotagged.bam"
    whatshap_bridge_haplotagged_sort_bam = os.path.join(bridge_dir, "whatshap_bridge") + ".haplotagged_sort.bam"

    whatshap_haplotagged_bam = os.path.join(whatshap_dir, "whatshap") + ".haplotagged.bam"
    whatshap_haplotagged_sort_bam = os.path.join(whatshap_dir, "whatshap") + ".haplotagged_sort.bam"

    margin_vcf = os.path.abspath(os.path.join(margin_dir, "MARGIN_PHASED.phased.vcf"))
    margin_vcf_gz = os.path.abspath(os.path.join(margin_dir, "MARGIN_PHASED.phased.vcf.gz"))
    ...
    if (os.path.isfile(filtered_bam) or os.path.isfile(haplotagged_bam)) and not overwrite:
        ...
        index_cmd = [SAMTOOLS, "index", "-@40", filtered_bam]
    ...
    if os.path.isfile(pepper_vcf) and not overwrite:
        ...
        pepper_cmd = [SINGULARITY,"run", PEPPER_MIRROR, "run_pepper_margin_deepvariant", "call_variant", "-b", os.path.abspath(filtered_bam), "-f", os.path.abspath(args.assembly),
                      "-o", os.path.abspath(pepper_dir), "-m", model_copy, "-t", str(args.threads), "-s", "Sample", reads_arg,
                      "--pepper_include-supplementary", "--no_pepper_quantized", "--only_haplotag", "2>&1", "|tee", pepper_log]
    ...
    if os.path.isfile(margin_vcf) and not overwrite:
        ...
        margin_cmd = [SINGULARITY,"run", MARGIN_MIRROR, "margin phase", os.path.abspath(filtered_bam), os.path.abspath(args.assembly), 
                      pepper_vcf, MARGIN_CONFIG[args.rtype],
        ...
        index_cmd = [SAMTOOLS, "index", "-@40", haplotagged_bam]
        logger.info("Running: %s", " ".join(index_cmd))
        subprocess.check_call(" ".join(index_cmd), shell=True)
        overwrite = True

    #STAFE 3.1 Tag with WhatsHap:
    if os.path.isfile(whatshap_haplotagged_sort_bam) and not overwrite:
        logger.info("Skipped whatshap phase")
    else:
        whatshap_log = os.path.join(whatshap_dir, "whatshap.log")
        if not os.path.isdir(whatshap_dir):
            os.mkdir(whatshap_dir)
        gzip_vcf(margin_vcf)
        whatshap_cmd = [WHATSHAP, "haplotag", "-o", whatshap_haplotagged_bam, "--output-threads=40", "--reference", os.path.abspath(args.assembly), 
                        margin_vcf_gz, os.path.abspath(filtered_bam), "--ignore-read-groups", "2>&1", "|tee", whatshap_log]
        logger.info("Running: %s", " ".join(whatshap_cmd))
        subprocess.check_call(" ".join(whatshap_cmd), shell=True)
        file_check(whatshap_haplotagged_bam)
        sort_cmd = [SAMTOOLS, "sort", "-@40", "-o", whatshap_haplotagged_sort_bam, whatshap_haplotagged_bam]
        logger.info("Running: %s", " ".join(sort_cmd))
        subprocess.check_call(" ".join(sort_cmd), shell=True)
        index_cmd = [SAMTOOLS, "index", "-@40", whatshap_haplotagged_sort_bam]
        logger.info("Running: %s", " ".join(index_cmd))
        subprocess.check_call(" ".join(index_cmd), shell=True)

        overwrite = True

    #STAFE 3.2 bridge vcf file 
    if os.path.isfile(bridge_haplotagged_bam) and not overwrite:
        logger.info("Skipped bridge phase")
    else:
        bridge_log = os.path.join(bridge_dir, "bridge.log")
        if not os.path.isdir(bridge_dir):
            os.mkdir(bridge_dir)
        gzip_vcf(margin_vcf)
        bridge_cmd = [PYTHON, "BayesPhase Path", margin_vcf, whatshap_haplotagged_sort_bam, 
                      bridge_vcf, bridge_haplotagged_bam, "-t", str(args.threads),"2>&1", "|tee", bridge_log]
        logger.info("Running: %s", " ".join(bridge_cmd))
        subprocess.check_call(" ".join(bridge_cmd), shell=True)
        file_check(bridge_haplotagged_bam)
        index_cmd = [SAMTOOLS, "index", "-@40", bridge_haplotagged_bam]
        logger.info("Running: %s", " ".join(index_cmd))
        subprocess.check_call(" ".join(index_cmd), shell=True)
        overwrite = True

    # #STAFE 3.3 whatshap tag bridge vcf file 
    # if os.path.isfile(whatshap_bridge_haplotagged_sort_bam) and not overwrite:
    #     logger.info("Skipped whatshap_bridge phase")
    # else:
    #     whatshap_log = os.path.join(bridge_dir, "whatshap.log")
    #     gzip_vcf(bridge_vcf)
    #     whatshap_cmd = [WHATSHAP, "haplotag", "-o", whatshap_bridge_haplotagged_bam, "--output-threads=40", "--reference", os.path.abspath(args.assembly), 
    #                     bridge_vcf_gz, os.path.abspath(filtered_bam), "--ignore-read-groups", "2>&1", "|tee", whatshap_log]
    #     logger.info("Running: %s", " ".join(whatshap_cmd))
    #     subprocess.check_call(" ".join(whatshap_cmd), shell=True)
    #     file_check(whatshap_bridge_haplotagged_bam)
    #     sort_cmd = [SAMTOOLS, "sort", "-@40", "-o", whatshap_bridge_haplotagged_sort_bam, whatshap_bridge_haplotagged_bam]
    #     logger.info("Running: %s", " ".join(sort_cmd))
    #     subprocess.check_call(" ".join(sort_cmd), shell=True)
    #     index_cmd = [SAMTOOLS, "index", "-@40", whatshap_bridge_haplotagged_sort_bam]
    #     logger.info("Running: %s", " ".join(index_cmd))
    #     subprocess.check_call(" ".join(index_cmd), shell=True)
    ...
    if all(map(os.path.isfile, polished_flye_hap.values())) and not overwrite:
        ...
        def run_flye_hp(hp):
            ...
            flye_cmd = [FLYE, "--polish-target", os.path.abspath(args.assembly), reads_arg,  bridge_haplotagged_bam, "-t", str(threads),
    ...
    find_breakpoints(bridge_haplotagged_bam, structural_dir, args.threads)
    ...
    for hp in [1, 2]:
        ...
        minimap_cmd = [MINIMAP, "-ax", "asm5", "-t", str(args.threads), "-K", "5G", "-I", "64G",
                       ...
                       SAMTOOLS, "sort", "-m", "4G", "-@40", ">", minimap_out]
        logger.info("Running: %s", " ".join(minimap_cmd))
        subprocess.check_call(" ".join(minimap_cmd), shell=True)
        subprocess.check_call(SAMTOOLS + " index -@ 40 {0}".format(minimap_out), shell=True)
